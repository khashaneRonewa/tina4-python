...
DB_CONNECTIONS = [
    ("mysql.connector:localhost/3307:test_db", "root", "secret", "mysql.connector"),
]

@pytest.fixture(
    params=DB_CONNECTIONS,
    ids=["mysql"],
    scope="module"
)
def db(request):
    conn_str, user, passwd, driver_module = request.param

    try:
        importlib.import_module(driver_module)
    except ImportError as e:
        pytest.fail(f"[{driver_module}] not installed: {e}")

    try:
        database = Database(conn_str, user, passwd)
        database.execute("SELECT 1")
        return database
    except Exception as e:
        pytest.fail(f"[{driver_module}] connection failed: {e}")


def test_DBMAIN_001_mysql_connection(db):
    """DBMAIN-001: MySQL connection"""
